class User::HomeController < ApplicationController
  ...
  def top
    ...
    # 誕生日
    @birthday_members = Member.where("cast(strftime('%m', birthday) as int) = ?", Time.now.month).sort_by { |member| member.birthday.day }
    @next_birthday_members = Member.where("cast(strftime('%m', birthday) as int) = ?", Time.now.next_month.month).sort_by { |member| member.birthday.day }
    # 投稿
